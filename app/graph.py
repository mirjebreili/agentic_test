...
# --- Error Handling ---
def error_handler_node(state: TraderState) -> TraderState:
    """A node to handle and log errors, terminating the graph."""
...
async def get_candles(instrument: str, timeframe: str, count: int = 200) -> str:
    """
    Asynchronously get recent OHLC candles. Returns JSON string of candles or an error dict.
    """
    try:
        ...
        if settings.broker_provider == "paper" and not df.empty:
            ...
            last_bar = df.iloc[-1]
            PaperBroker().on_bar(instrument, float(last_bar.open), float(last_bar.high), float(last_bar.low), float(last_bar.close))

        return json.dumps(df.to_dict(orient="records"))
    except Exception as e:
        return json.dumps({"error": f"Failed to get candles for {instrument}/{timeframe}: {e}"})

@tool
def execute_order(order: dict, open_positions: int = 0, daily_dd: float = 0.0, allow_new_entries: bool = True) -> dict:
    """Execute order via broker, with error handling."""
    try:
        ...
        return {"error": f"Failed to execute order: {e}"}

# --- Graph Builder ---
def build_trader_graph(config: dict):
    llm = make_llm()

    # Tools that don't need error handling as they are internal
    @tool
    ...
    strategy_agent = create_react_agent(
        ...
        tools=[get_candles],
        name="strategy_agent",
        prompt="""--- Entering Strategy Node ---
You are the Strategy Selector. Your job is to analyze the market and decide on a trading strategy.
...
3. Return a JSON object with your chosen preset and a brief rationale. Example: {"preset": "trend_following", "rationale": "The 20-period EMA is above the 50-period EMA."}""",
    ...
    signal_agent = create_react_agent(
        ...
        tools=[propose_order],
        name="signal_agent",
        prompt="""--- Entering Signal Node ---
You are the Signal Agent. Based on the strategy preset and candle data from the previous step, generate a trading signal.
...
If you decide to trade, call the `propose_order` tool with the signal details.""",
    ...
    risk_agent = create_react_agent(
        ...
        tools=[attach_stops],
        name="risk_agent",
        prompt="""--- Entering Risk Node ---
You are the Risk Agent. Take the proposed order and attach stop_loss and take_profit levels based on the latest Average True Range (ATR) from the candle data.
...
Reply with the final order JSON, including the calculated stop_loss and take_profit.""",
    ...
    exec_agent = create_react_agent(
        ...
        tools=[execute_order],
        name="exec_agent",
        prompt="""--- Entering Execution Node ---
...
If the tool returns an error or a "skipped" status, report it. Otherwise, confirm the successful execution.""",
    ...
    graph.add_conditional_edges(
        ...
        {"continue": "signal", "error": "error_handler"}
    )
